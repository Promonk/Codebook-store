...
export async function getUserLibrary(){
    ...
    const response = await fetch(`${process.env.REACT_APP_HOST}/library?user.id=${browserData.cbid}`, requestOptions);
    if(!response.ok){
        throw { message: response.statusText, status: response.status }; //eslint-disable-line
    }
    const data = await response.json();
...
export async function createOrder(cartList: any, total: number, user: any){
    const browserData = getSession();
    const order = {
        cartList: cartList,
        amount_paid: total,
